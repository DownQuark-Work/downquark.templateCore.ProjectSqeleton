...
switch(Deno.args[0]) {
  case 'module':
    runCwd = '../application/scripts/_modules'
    const dest = relative(runCwd,'../network/_assets/js/_v1/_modules')
    const moduleScript = `${Deno.args[1].replace(/-/g,'/')}.ts`
    const destScript = `${dest}/${Deno.args[1]}.js`
    runCmd = `deno bundle -c ${relative(runCwd,'../network/deno.jsonc')} ${moduleScript} ${destScript}`
    break
  default :
